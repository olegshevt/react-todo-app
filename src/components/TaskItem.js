...
function TaskItem({ name, selectedTask, index, completed, removedTask, taskEdit }) {
  const handleRemove = () => {
    removedTask(index);
  };
  ...
  const handleEdit = () => {
    const newTaskText = window.prompt('Your task:', name);
    taskEdit(newTaskText, index);
  };

  return (
    <div className={`todo__list-item ${completed ? 'todo__list-item--completed' : ''}`}>
      <div onClick={handleSelect} className="todo__list-item-check">
        <svg
          width="11"
          height="8"
          viewBox="0 0 11 8"
      ...
      <div onClick={handleRemove} className="todo__list-item-remove">
        <svg
          width="11"
          height="11"
          viewBox="0 0 11 11"
